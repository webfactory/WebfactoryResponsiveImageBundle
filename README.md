...
- `options`: an array with the following keys:
    ...
    - `transformations_to_widths` (optional): an object with thumbor transformation names as keys and image widths as values. Is used to define the `srcset`-Attribute. Defaults to a `srcset` with keys from `imagex_xxxs` to `image_xl` respectively to values from `160w` to `1600w`.
